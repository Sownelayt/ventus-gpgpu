...
object Instructions {
  ...
  def CP_ASYNC_BULK      = BitPat("b00000?????????????001?????1000010")
  def CP_ASYNC_TENSOR    = BitPat("b00000?????????????010?????1000010")
  // inst[31:29] selects copy/reduce and inst[28:27] selects the 32-bit
  // reduce type.  The ingress validates legal redOp/type combinations.
  def CP_ASYNC_BULK_S2G  = BitPat("b?????????????????011?????1000010")
  def CP_ASYNC_TENSOR_S2G = BitPat("b???00????????????100?????1000010")
  def PREFETCH_TENSORMAP = BitPat("b00000?????????????101?????1000010")
  def INVALIDATE_TENSORMAP = BitPat("b00001?????????????101?????1000010")
  def CP_ASYNC_S2G_GROUP = BitPat("b00000?????????????110?????1000010")
  def CP_ASYNC_MBARRIER_PROXY = BitPat("b00000?????????????111?????1000010")
}
